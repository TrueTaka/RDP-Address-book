...
def load_connections():
    ...
    for name in base_connection:
        connection_listbox.insert(tk.END, f"{name}")
...
def add_connection():
    ...
    def add_connection_confirm():
        ...
        # Добавляем новый элемент в список подключений
            connection_listbox.insert(tk.END, name)
...
def edit_connection():
    ...
    try:
        ...
        address = base_connection[selected_name]
    except tk.TclError:
        messagebox.showerror("Ошибка", "Выберите подключение для изменения данных")
        return
    except KeyError:
        messagebox.showerror("Ошибка", f"Подключение '{selected_name}' не найдено")
        return
...
connection_listbox.pack(fill="both", expand="yes", padx=5, pady=5)
# Загрузка подключений из файла
load_connections()
...
connect_button = CTkButton(button_frame, text=" Изменить ", command=edit_connection)
